    ...
    if (!builder.ctx.commandCtx.skipProjectConfiguration) {
      try {
        ...
        switch (builder.ctx.platform) {
          case 'android':
            isUpdatesConfigured = await isUpdatesConfiguredAndroidAsync(
              builder.ctx.commandCtx.projectDir
            );
            break;
          case 'ios':
            isUpdatesConfigured = await isUpdatesConfiguredIOSAsync(
              builder.ctx.commandCtx.projectDir
            );
            break;
        }
